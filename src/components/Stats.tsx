        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {stats.map((stat, index) => (
            <div key={index} className="text-center bg-white/70 rounded-2xl p-6 border border-black/5 shadow-sm">
              ...
              <div className="text-3xl md:text-4xl font-semibold text-[#0b0b0b] mb-2 tracking-tight">{stat.value}</div>
              <div className="text-sm uppercase tracking-[0.2em] text-black/50">{stat.label}</div>
            </div>
          ))}
        </div>
      </div>
